...
export default async function ClinicDetailPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ reported?: string; deleted?: string; shared?: string }> }) {
  ...
  const structuredData = { "@context": "https://schema.org", "@type": "WebPage", name: `${clinic.hospital.name} ${clinic.branch} Yorumları`, url: `${siteUrl}/clinic/${id}`, about: { "@type": "Hospital", name: clinic.hospital.name, address: { "@type": "PostalAddress", addressLocality: clinic.hospital.district, addressRegion: clinic.hospital.city, addressCountry: "TR" } } };
