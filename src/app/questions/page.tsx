...
export default async function QuestionsPage({ searchParams }: { searchParams: Promise<{ sent?: string; answered?: string; error?: string }> }) {
  ...
  const reviewIds = Array.from(new Set((questions ?? []).map((q) => q.review_id)));
  ...
  const incoming = (questions ?? []).filter((q) => q.author_doctor_id === auth.user.id);
  const outgoing = (questions ?? []).filter((q) => q.asker_doctor_id === auth.user.id);

  const QuestionCard = ({ question, incomingCard }: { question: any; incomingCard: boolean }) => {
