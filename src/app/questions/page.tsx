...
export default async function QuestionsPage({ searchParams }: { searchParams: Promise<{ sent?: string; answered?: string; error?: string }> }) {
  ...
  if (reviewIds.length) {
    ...
    if (clinicIds.length) {
      ...
      const clinicById = new Map((clinics ?? []).map((clinic) => [clinic.id, { hospitalName: hospitalNameById.get(clinic.hospital_id) ?? "Hastane", branch: clinic.branch, clinicId: clinic.id }]));
  ...
  return <Container className="py-12 sm:py-16"><main className="mx-auto max-w-2xl space-y-10">
    ...
    <section className="space-y-4"><h2 className="text-base font-semibold">Bana gelen sorular ({incoming.length})</h2>{!incoming.length ? <p className="rounded-lg border p-4 text-sm text-muted-foreground">Henüz deneyimleriniz hakkında gönderilmiş bir soru yok.</p> : incoming.map((question) => { const context = contextByReview.get(question.review_id); return <article key={question.id} className="space-y-4 rounded-lg border bg-card p-5 shadow-sm"><div><div className="flex flex-wrap items-center justify-between gap-2"><p className="text-xs font-medium text-muted-foreground">{contextLabel(context)}</p>{context && <Link href={`/clinic/${context.clinicId}`} className="text-xs font-medium text-primary hover:underline">Deneyimi gör</Link>}</div><p className="mt-2 text-sm leading-relaxed">{question.question}</p></div>{question.answer ? <div className="rounded-md bg-muted p-3"><p className="text-xs font-medium text-muted-foreground">Yanıtınız</p><p className="mt-1 text-sm leading-relaxed">{question.answer}</p></div> : <form action={answerReviewQuestion} className="space-y-3"><input type="hidden" name="questionId" value={question.id} /><Textarea name="answer" minLength={10} maxLength={1500} required placeholder="Deneyiminize dayanarak kısa ve net yanıtlayın..." /><Button type="submit" size="sm">Yanıtı gönder</Button></form>}</article>; })}</section>

    <section className="space-y-4"><h2 className="text-base font-semibold">Sorduğum sorular ({outgoing.length})</h2>{!outgoing.length ? <p className="rounded-lg border p-4 text-sm text-muted-foreground">Bir deneyimin altındaki “Bu deneyim hakkında sor” düğmesiyle soru gönderebilirsin.</p> : outgoing.map((question) => { const context = contextByReview.get(question.review_id); return <article key={question.id} className="space-y-3 rounded-lg border bg-card p-5 shadow-sm"><div className="flex flex-wrap items-center justify-between gap-2"><p className="text-xs font-medium text-muted-foreground">{contextLabel(context)}</p>{context && <Link href={`/clinic/${context.clinicId}`} className="text-xs font-medium text-primary hover:underline">Deneyimi gör</Link>}</div><div><p className="text-xs font-medium text-muted-foreground">Sorun</p><p className="mt-1 text-sm leading-relaxed">{question.question}</p></div><div className="rounded-md bg-muted p-3"><p className="text-xs font-medium text-muted-foreground">Hekimin yanıtı</p><p className="mt-1 text-sm leading-relaxed">{question.answer ?? "Deneyimi paylaşan hekim henüz yanıt vermedi."}</p></div></article>; })}</section>
